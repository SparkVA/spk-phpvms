...
if($pirepID === array())
{
    error_log("Pirep found for bid: ".$_POST['bidID']);
    ...
    (id, user_id, airline_id, flight_id, flight_number, route_code, route_leg, flight_type, dpt_airport_id, arr_airport_id, alt_airport_id, level, planned_distance, planned_flight_time, route, source, source_name, status, state,created_at, updated_at)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 1, "smartCARS 3", "BST", 0, NOW(), NOW())',
    ...
    error_log("Pirep not found for bid: ".$_POST['bidID']);
